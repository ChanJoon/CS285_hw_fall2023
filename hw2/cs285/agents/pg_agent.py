...
class PGAgent(nn.Module):
    ...
    def _estimate_advantage(
        ...
        if self.normalize_advantages:
            # To avoid division by zero, add a small constant to the standard deviation
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        gamma_r = [self.gamma ** t * rewards[t] for t in range(len(rewards))]
        total_return = np.sum(gamma_r)
        discounted_return = [total_return for _ in range(len(rewards))]

...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.02) -> int:
        ...
        action = torch.randint(0, self.num_actions,size=(1,)) \
                if np.random.rand() < epsilon \
                else self.critic(observation).argmax() 
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values = reward + self.discount * next_q_values * (1 - done.float()) # (batch_size,)
